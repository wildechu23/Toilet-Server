...
const db = new sqlite3.Database('./toilets.db', (err) => {
	if (err) {
...
interface LocationDataProps {
	...
	lat: number,
	lng: number,
	restrooms: RestroomProps[],
}

// POST new location
app.post('/locations', (req, res) => {
    const { name, lat, lng, restrooms }: LocationDataProps = req.body;
    ...
    db.run(sql, [name, lat, lng], function (err) {
        if (err) {
            ...
            `;
            const restroomInsertions = restrooms.map(r => new Promise<void>((resolve, reject) => {
                db.run(
                    restroomQuery,
                    [locationId, r.gender, r.single_stall, r.wheelchair_stall, r.mirrors, r.hand_dryers, r.paper_towels],
                    function (err) {
                        if (err) {
                            reject(err);
                        } else {
    ...
    db.run(updateLocationQuery, [name, lat, lng, id], function (err) {
        ...
        db.run(deleteRestroomsQuery, [id], function (err) {
            ...
            const restroomInsertions = restrooms.map(r => new Promise<void>((resolve, reject) => {
                db.run(
                    restroomInsertQuery,
                    [id, r.gender, r.single_stall, r.wheelchair_stall, r.mirrors, r.hand_dryers, r.paper_towels],
                    function (err) {
                        if (err) {
                            reject(err);
                        } else {
